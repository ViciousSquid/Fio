...
class LogicThread(threading.Thread):
    ...
    def __init__(self, game_state: ThreadedGameState, 
        ...
        # Active occupants keyed by trigger id, then (filter type, entity id).
    ...
    def _poll_triggers(self, use_key_pressed=False):
        """Poll all trigger occupancy once per second.

        Player, prop, and monster detection all happen here. The logic thread
        tick only accumulates elapsed time and latches the use-key edge; it does
        not inspect trigger occupancy at 60 Hz.

        Candidate prop/monster positions and trigger AABBs are evaluated as
        NumPy matrices. Python is used only for the trigger list setup and the
        sparse enter/exit transition events generated by the 1 Hz sample.
        """
        if not self.player:
            return
        ...
        # Build the trigger matrix once for this poll. Touch triggers are
        # eligible for player/prop/monster occupancy; use triggers remain a
        # player interaction and are evaluated in the player pass below.
        for bid, brush in self._trigger_brushes:
            ...
            if activation != 'use' and filters & {'player', 'props', 'monsters'}:
                bounds = brush_aabb_bounds(brush)
                slot = len(trigger_entries)
                trigger_entries.append((bid, brush, bounds))
                if 'props' in filters:
                    prop_trigger_slots.append(slot)
                if 'monsters' in filters:
                    monster_trigger_slots.append(slot)

        # Player touch detection is now sampled at the same 1 Hz rate as every
        # other trigger category.
        for slot, (bid, brush, bounds) in enumerate(trigger_entries):
        ...
        # Use triggers are also sampled at 1 Hz. The pending key edge is latched
        # by _handle_triggers so a brief press survives until this sample.
        for bid, brush in self._trigger_brushes:
            if brush.get('disabled', False):
                continue
            ...
            if glm.distance(player_pos, t_pos) >= use_radius:
                continue
            ...
            if glm.length(offset) <= 0.0001:
                ...
                facing = glm.dot(p_forward, to_trigger) > 0.5
            ...
            if use_key_pressed and not already_fired:
        ...
        def poll_category(entities, trigger_slots, category):
            ...
            trigger_bounds = np.asarray(
                [trigger_entries[i][2] for i in trigger_slots],
                dtype=np.float32,
            )
        ...
        # Player enter/exit transitions use the same 1 Hz sample.
        ...
        # Continuous hurt triggers are also evaluated only at the trigger poll
        # cadence. A 1 Hz trigger update means a 1 Hz sustained-damage cadence.
        for bid in current_player_contacts:
            brush = self._trigger_brush_by_bid.get(bid)
