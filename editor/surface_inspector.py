...
class SurfaceInspector(QDialog):
    ...
    def _build_ui(self):
        ...
        self.hshift, self.hshift_step = self._add_row(
            grid, 1, "Horizontal shift", -8192.0, 8192.0, 3, 0.0, 0.125)
        self.vshift, self.vshift_step = self._add_row(
            grid, 2, "Vertical shift", -8192.0, 8192.0, 3, 0.0, 0.125)
        self.hstretch, self.hstretch_step = self._add_row(
            grid, 3, "Horizontal scale", -64.0, 64.0, 3, 1.0, 0.5)
        self.vstretch, self.vstretch_step = self._add_row(
            grid, 4, "Vertical scale", -64.0, 64.0, 3, 1.0, 0.5)
        self.rotate, self.rotate_step = self._add_row(
            grid, 5, "Rotate", -360.0, 360.0, 2, 0.0, 45.0)
        ...
        self.natural_btn.clicked.connect(self._apply_natural)
    ...
    def refresh_from_face(self):
        ...
        if not self.target:
            return
        brush, face_key = self.target
        transform = ft.get_transform(brush, face_key)
        self._loading = True
        try:
    ...
    def _on_value_changed(self, *_):
        ...
        for brush, face_key in self.target_faces():
            ft.set_transform(brush, face_key, shift=shift, scale=scale,
                             angle=angle)
        self._commit()
    ...
    def _apply_natural(self):
        """One texel per world unit on each target face."""
        if not self.target:
            return
        self._begin_undo_burst()
        for brush, face_key in self.target_faces():
