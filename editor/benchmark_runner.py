...
This module deliberately talks to the existing Fio MainWindow. It does not add
benchmark code to the renderer or create a second engine instance.
"""
...
class BenchmarkRunner:
    """Own the benchmark state machine while keeping UI in BenchmarkDialog."""
    ...
    def __getattr__(self, name):
        # Keep the extracted workload/reporting code close to its original
        # self.* calls without introducing a general task framework.
...
def _finish_live_stress_result(self, label, metrics):
    ...
    # The following are the original sequencing entry points, retained under
    # their existing private names so the surrounding editor code remains small.
def _start(self):
    if self._running:
        return
